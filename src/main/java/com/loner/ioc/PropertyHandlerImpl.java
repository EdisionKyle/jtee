/**
 *  工程名：jtee
 *  文件名：PropertyHandlerImpl.java
 *  包名：com.loner.ioc
 *  创建时间：2016年11月16日 上午10:40:45
...
import com.loner.ioc.exception.BeanCreateException;
import com.loner.ioc.exception.PropertyException;

/**
 *  类名：PropertyHandlerImpl
...
public class PropertyHandlerImpl implements PropertyHandler {
	...
	public Object setProperties(Object obj, Map<String, Object> properties) {
		...
		try {
			...
			for (String key : properties.keySet()) {
				/**
				 * 调用本类中定义的getSetterMethodName方法获得一个属性的成员变量 对应的set方法
				 */
	...
	/**
	 * 得到obj对象中的所有setXXX方法的Map映射，Map的key值为对应的属性名，value为对应的set 方法的Method对象
	 */
	public Map<String, Method> getSetterMethodsMap(Object obj) {
		...
		/**
		 * 定义一个结果的映射用来存放方法的属性名（对应到bean里面就是bean的id属性的值） 与对应的set方法
		 */
		Map<String, Method> result = new HashMap<String, Method>();
		/**
		 * 遍历所有的Method对象，调用本类的getMethodNameWithoutSet得到该方法 对应的属性名（也就是去掉set后的值）
		 */
		for (Method m : methods) {
	...
	private String getMethodNameWithOutSet(String methodName) {
		 ...
		 * 到底注入什么参数实际上是看setXxx方法去掉set然后再把后面的第一个字符变成小写之后的xxx 作为依据如一个类中有属性 private
		 * String yy; public void setXx(String aa) { this.yy = aa; }
		 * <bean id="test3" class="com.tear.Test3"> <property name="xx"> <value
		 * type="java.lang.String">123456</value> </property> </bean>
	 ...
	 * 其中object为带设值的对象，argBean就是要设进去的参数，第三个参数就是setXX对象本身的Method对象，主要是 方便使用反射
	 */
	public void executeMethod(Object object, Object argBean, Method method) {
		try {
			...
			/**
			 * 如果参数数量为1，则执行该方法，因为作为setXX方法参数个数肯定 是1
			 */
